"""PHASE2-WAGLE-R2B1-SERVICE-ACTOR-AND-BINDING-001 real-PostgreSQL suite.
...
async def test_05_different_family_binding_blocked(service_env):
    ...
    resp = await client.post(
        f"/api/families/{other_family_id}/wagle/service/actions",
...
# ---------------------------------------------------------------------------
# 6. inactive Family Wagle subscription blocks publish
# ---------------------------------------------------------------------------
...
async def test_07_inactive_binding_blocked(service_env):
    client, db, family_id = service_env["client"], service_env["db"], service_env["family_id"]
    svc = service_env["service"]
    await wagle_service.set_service_binding_status(db, svc.binding_id, "inactive")
    resp = await client.post(
        f"/api/families/{family_id}/wagle/service/actions",
        json={"room_id": str(svc.room_id), "action_type": "mission_approved", "schema_version": 1,
              "source": "mission-svc", "source_event_id": "evt-1", "snapshot": {}},
...
#    fn_wagle_service_binding_room_guard (migration 0004) now refuses to let
...
async def test_08_non_service_room_publish_blocked(service_env):
    ...
    with pytest.raises(Exception, match="must reference a SERVICE Room"):
        await db.execute(
            text("INSERT INTO wagle_service_bindings (service_principal_id, family_group_id, room_id, allowed_actions, status) "
...
async def test_10_normal_service_action_publish(service_env):
    ...
    row = (await db.execute(text("SELECT message_type, sender_participant_id, service_principal_id FROM wagle_messages WHERE id = :id"), {"id": body["id"]})).one()
...
async def test_11_sequential_replay_idempotent(service_env):
    ...
    first = await client.post(f"/api/families/{family_id}/wagle/service/actions", json=payload, headers=svc.headers)
    second = await client.post(f"/api/families/{family_id}/wagle/service/actions", json=payload, headers=svc.headers)
    third = await client.post(f"/api/families/{family_id}/wagle/service/actions", json=payload, headers=svc.headers)
    ...
    count = (await db.execute(
        text("SELECT count(*) FROM wagle_messages WHERE service_principal_id = :pid AND source = 'mission-svc' AND source_event_id = 'evt-replay'"),
...
async def test_12_concurrent_replay_idempotent(service_env):
    ...
    async def send():
        return await client.post(f"/api/families/{family_id}/wagle/service/actions", json=payload, headers=svc.headers)
    ...
    count = (await db.execute(
        text("SELECT count(*) FROM wagle_messages WHERE service_principal_id = :pid AND source = 'mission-svc' AND source_event_id = 'evt-concurrent'"),
        {"pid": svc.principal_id},
    )).scalar_one()
    assert count == 1

    audit_result_codes = list((await db.execute(
        text("SELECT result_code FROM wagle_service_audit_log WHERE service_principal_id = :pid AND room_id = :rid AND event_type IN ('publish_success', 'replay_detected') ORDER BY created_at"),
...
async def test_13_same_key_different_payload_conflict(service_env):
    ...
    row = (await db.execute(
        text("SELECT count(*), max((service_payload->>'snapshot')::text) FILTER (WHERE service_payload->'snapshot'->>'points' = '5') "
             "FROM wagle_messages WHERE service_principal_id = :pid AND source_event_id = 'evt-conflict'"),
        {"pid": svc.principal_id},
    )).one()
    assert row[0] == 1
...
async def test_14_denied_request_db_invariant(service_env):
    client, db, family_id = service_env["client"], service_env["db"], service_env["family_id"]
    svc = service_env["service"]
    before = await _counts(db, "wagle_messages", "wagle_service_bindings", "service_principals")
    resp = await client.post(
        f"/api/families/{family_id}/wagle/service/actions",
    ...
    assert resp.status_code == 422
    after = await _counts(db, "wagle_messages", "wagle_service_bindings", "service_principals")
    assert before == after
...
async def test_15_audit_log_excludes_secrets_and_body(service_env):
    ...
    rows = list((await db.execute(text("SELECT event_type, result_code, detail FROM wagle_service_audit_log"))).all())
    assert len(rows) >= 1
    dump = str(rows)
...
async def test_scenario_A_normal_publish_into_bound_room(service_env):
    ...
    await db.execute(
        text(
            "INSERT INTO wagle_participants (family_group_id, room_id, family_membership_id, room_role, status, joined_sequence) "
    ...
    visible = await client.get(
        f"/api/families/{family_id}/wagle/rooms/{svc.room_id}/messages",
...
async def test_scenario_C_resend_creates_no_duplicate(service_env):
    ...
    first = await client.post(f"/api/families/{family_id}/wagle/service/actions", json=payload, headers=svc.headers)
    resend = await client.post(f"/api/families/{family_id}/wagle/service/actions", json=payload, headers=svc.headers)
    assert first.status_code == 201 and resend.status_code == 201
    assert first.json()["id"] == resend.json()["id"]
    count = (await db.execute(text("SELECT count(*) FROM wagle_messages WHERE source_event_id = 'evt-scenario-c'"))).scalar_one()
    assert count == 1
