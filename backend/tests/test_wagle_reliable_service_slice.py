"""PHASE2-WAGLE-R2B2-RELIABLE-SERVICE-SLICE-001 real-PostgreSQL suite.
...
(18-22), and a full Mark Point -> Outbox -> Worker -> Wagle -> user-visible
...
async def test_outbox_00_same_mission_eight_concurrent_completions_are_serialized(family_env):
    ...
    message_count = (
        await db.execute(
            text(
                "SELECT count(*) FROM wagle_messages dm JOIN service_outbox_events o "
...
async def test_outbox_00_recompletion_has_two_distinct_occurrence_events(family_env):
    ...
    assert (await db.execute(text("SELECT count(*) FROM wagle_messages WHERE family_group_id = :id"), {"id": family_id})).scalar_one() == 2
...
async def test_outbox_04_worker_publishes_marks_published(family_env):
    db, member, family_id = family_env["db"], family_env["member"], family_env["family_id"]
    # family_env already has an active Wagle subscription.
    await _complete_mission(db, member.player_id)
    result = await worker.run_once(db)
    assert result["published"] == 1
    rows = await _outbox_rows(db, family_id)
    assert rows[0]["status"] == "PUBLISHED"
    msg = (await db.execute(text("SELECT message_type, service_code FROM wagle_messages WHERE family_group_id = :fid"), {"fid": family_id})).one()
...
async def test_outbox_05_transient_failure_then_retryable(family_env):
    db, member, family_id = family_env["db"], family_env["member"], family_env["family_id"]
    # Suspend the Wagle subscription so the first delivery attempt fails in a
    # way the Worker must treat as retryable, not permanent.
    await db.execute(text("UPDATE service_subscriptions SET status = 'suspended' WHERE family_group_id = :fid AND service_code = 'wagle'"), {"fid": family_id})
    await db.commit()
    await _complete_mission(db, member.player_id)
    result = await worker.run_once(db)
    ...
    await db.execute(text("UPDATE service_subscriptions SET status = 'active' WHERE family_group_id = :fid AND service_code = 'wagle'"), {"fid": family_id})
...
async def test_outbox_09_and_10_publish_then_crash_then_converges(family_env):
    ...
    # Simulate: Wagle already durably has the message (call the same publish
    ...
    principal = await wagle_service.bootstrap_service_principal(db, "mark-point", "Mark Point")
    binding = await wagle_service.ensure_canonical_service_binding(db, principal, family_id, worker.ALLOWED_ACTIONS_BY_OWNER["mark-point"])
    ...
    await wagle_service.publish_service_action(db, principal, family_id, publish_data)
    count_before_retry = (await db.execute(text("SELECT count(*) FROM wagle_messages WHERE family_group_id = :fid"), {"fid": family_id})).scalar_one()
    assert count_before_retry == 1

    # Retry: the Worker doesn't know delivery already happened - it retries
    # with the same stored source_event_id, hits Wagle idempotency, and this
    ...
    count_after_retry = (await db.execute(text("SELECT count(*) FROM wagle_messages WHERE family_group_id = :fid"), {"fid": family_id})).scalar_one()
...
async def test_room_11_self_onboarding(family_env):
    ...
    resp = await client.post(f"/api/families/{family_id}/wagle/services/mark-point/room", headers=member.headers)
...
async def test_room_12_repeat_call_one_participant(family_env):
    ...
    first = await client.post(f"/api/families/{family_id}/wagle/services/mark-point/room", headers=member.headers)
    second = await client.post(f"/api/families/{family_id}/wagle/services/mark-point/room", headers=member.headers)
    assert first.json()["participant_id"] == second.json()["participant_id"]
    count = (
        await db.execute(
            text("SELECT count(*) FROM wagle_participants WHERE room_id = :rid AND status = 'active'"),
...
async def test_room_13_other_family_denied(family_env):
    ...
    resp = await client.post(f"/api/families/{other_family_id}/wagle/services/mark-point/room", headers=member.headers)
...
async def test_room_14_inactive_membership_denied(family_env):
    ...
    resp = await client.post(f"/api/families/{family_id}/wagle/services/mark-point/room", headers=suspended.headers)
    assert resp.status_code == 403
...
async def test_room_16_inactive_binding_denied(family_env):
    db, client, member, family_id = family_env["db"], family_env["client"], family_env["member"], family_env["family_id"]
    _, binding = await _bind_mark_point(db, family_id)
    await wagle_service.set_service_binding_status(db, binding.id, "inactive")
    resp = await client.post(f"/api/families/{family_id}/wagle/services/mark-point/room", headers=member.headers)
...
async def test_room_17_non_service_room_binding_db_blocked(family_env):
    ...
    group_room_id = (
        await db.execute(
            text(
                "INSERT INTO wagle_rooms (id, family_group_id, room_type, status, created_by_actor_type, next_message_sequence, version) "
    ...
    with pytest.raises(Exception):
        await db.execute(
            text(
                "INSERT INTO wagle_service_bindings (service_principal_id, family_group_id, room_id, allowed_actions, status) "
...
async def test_limit_18_32kb_or_under_allowed(service_env):
    ...
    resp = await client.post(
        f"/api/families/{family_id}/wagle/service/actions",
        json={
...
async def test_limit_20_length_unaware_stream_still_capped():
    ...
    from app.domains.wagle.ingress_limits import enforce_service_body_limit, MAX_SERVICE_INGRESS_BYTES
...
async def test_limit_21_unknown_credential_runs_dummy_bcrypt(service_env, monkeypatch):
    from app.domains.wagle import service_actor
...
async def test_limit_22_wrong_secret_and_revoked_regression(service_env):
    ...
    await wagle_service.revoke_service_principal(db, svc.principal_id)
...
# ---------------------------------------------------------------------------
# E2E 23-28: 실제 미션 완료 -> Outbox -> Worker -> Wagle -> 사용자 조회
# ---------------------------------------------------------------------------
...
async def test_e2e_23_to_28_mission_completion_to_visible_message(family_env):
    ...
    onboard = await client.post(f"/api/families/{family_id}/wagle/services/mark-point/room", headers=member.headers)
    assert onboard.status_code == 200
    room_id = onboard.json()["room_id"]

    # 27. GET messages에서 mission.completed 확인
    visible = await client.get(f"/api/families/{family_id}/wagle/rooms/{room_id}/messages", headers=member.headers)
    ...
    count = (await db.execute(text("SELECT count(*) FROM wagle_messages WHERE family_group_id = :fid AND message_type = 'SERVICE_ACTION'"), {"fid": family_id})).scalar_one()
    assert count == 1
