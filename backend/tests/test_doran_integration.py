"""PHASE2-DORAN-FOUNDATION-R2A-REPAIR-001 real-PostgreSQL integration and
...
async def test_01b_cross_family_access_denied(family_env):
    ...
    room_resp = await client.post(
        f"/api/families/{family_env['family_id']}/doran/rooms",
        json={"room_type": "DIRECT", "target_membership_id": family_env["member"].membership_id},
        headers=family_env["admin"].headers,
    )
    room_id = room_resp.json()["id"]

    before = await _counts(db, "doran_messages")
    resp = await client.get(
        f"/api/families/{family_env['family_id']}/doran/rooms/{room_id}/messages",
        headers=outsider.headers,
    )
    assert resp.status_code == 403
    after = await _counts(db, "doran_messages")
    assert before == after


async def test_01c_non_participant_denied(family_env):
    """Same family, has a Doran role, but was never added to this room."""
    client, db, family_id = family_env["client"], family_env["db"], family_env["family_id"]
    ...
    resp = await client.get(
        f"/api/families/{family_id}/doran/rooms/{room_id}/messages",
...
async def test_02_direct_canonical_concurrent_creation(family_env):
    ...
    counts = await _counts(db, "doran_rooms", "doran_direct_pairs")
    assert counts["doran_rooms"] == 1
    assert counts["doran_direct_pairs"] == 1
...
async def test_03_message_sequence_concurrent_creation(family_env):
    ...
    count = (
        await db.execute(text("SELECT count(*) FROM doran_messages WHERE room_id = :rid"), {"rid": room_id})
    ).scalar_one()
    assert count == n
...
async def test_04_same_client_id_same_payload_concurrent_idempotent(family_env):
    ...
    row_count = (
        await db.execute(
            text(
                "SELECT count(*) FROM doran_messages WHERE room_id = :rid AND client_message_id = 'idem-1'"
            ),
            {"rid": room_id},
        )
    ).scalar_one()
...
async def test_05_same_client_id_different_payload_conflict_db_invariant(family_env):
    ...
    row = (
        await db.execute(
            text(
                "SELECT count(*), max(body) FILTER (WHERE body = 'original') FROM doran_messages "
    ...
    def factory(seq):
        async def put():
            return await client.put(
                f"/api/families/{family_id}/doran/rooms/{room_id}/read-state",
...
async def test_09_pagination_default_50_max_100_reject_101(family_env):
    ...
    default_resp = await client.get(
        f"/api/families/{family_id}/doran/rooms/{room_id}/messages", headers=admin.headers
    )
...
async def test_10_group_active_participant_cap_50_51(family_env):
    ...
    for mid in filler_ids:
        r = await client.post(
            f"/api/families/{family_id}/doran/rooms/{room_id}/participants",
    ...
    active_count = (
        await db.execute(
            text(
                "SELECT count(*) FROM doran_participants WHERE room_id = :rid AND status = 'active'"
            ),
            {"rid": room_id},
        )
    ).scalar_one()
...
async def test_12a_left_participant_visibility_capped_and_write_denied(family_env):
    ...
    leave = await client.post(
        f"/api/families/{family_id}/doran/rooms/{room_id}/leave", headers=member.headers
    )
...
async def test_12b_removed_participant_denied(family_env):
    ...
    participants = await client.get(
        f"/api/families/{family_id}/doran/rooms/{room_id}/participants", headers=admin.headers
    )
    ...
    remove = await client.delete(
        f"/api/families/{family_id}/doran/rooms/{room_id}/participants/{member_participant_id}",
        headers=admin.headers,
    )
    ...
    denied = await client.get(
        f"/api/families/{family_id}/doran/rooms/{room_id}/messages", headers=member.headers
    )
...
async def test_12c_subscription_read_only_allows_read_denies_write(family_env):
    ...
    await db.execute(
        text("UPDATE service_subscriptions SET status = 'suspended' WHERE family_group_id = :fid AND service_code = 'doran'"),
        {"fid": family_id},
    )
    await db.commit()
