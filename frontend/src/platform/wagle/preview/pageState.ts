import type { WaglePreviewPageState } from './types';

const PREVIEWABLE_STATES: readonly WaglePreviewPageState[] = ['normal', 'loading', 'empty', 'error'];
 ...
 * WaglePreviewPageState 원본 타입(A source contract)은 변경하지 않는다 —
 * 'read-only' 쿼리 값은 단순히 미지원 값과 동일하게 'normal'로 수렴할 뿐이다.
 */
export function resolvePreviewPageState(rawValue: string | null): WaglePreviewPageState {
