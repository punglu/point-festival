...
import {
  ...
  EmptyState,
  ErrorState,
} from '../doran/components';
import {
  doranPreviewRooms,
  doranPreviewRoomLabels,
  doranPreviewMessagesByRoom,
  doranPreviewDefaultConversation,
  doranPreviewServiceEventsByRoom,
  resolvePreviewPageState,
  type DoranPreviewRoom,
  type DoranPreviewPageState,
} from '../doran/preview';
import styles from './DoranLanding.module.css';

type ComposeStatus = 'idle' | 'pending' | 'failed';

interface ConversationProps {
  room: DoranPreviewRoom;
  onBack: () => void;
}
...
function Conversation({ room, onBack }: ConversationProps) {
  ...
  const messages = doranPreviewMessagesByRoom[room.id] ?? doranPreviewDefaultConversation;
  const serviceEvent = doranPreviewServiceEventsByRoom[room.id];
...
export function DoranLanding() {
  ...
  const selected = doranPreviewRooms.find((room) => room.id === roomIdParam) ?? null;
  ...
  const pageState: DoranPreviewPageState = serviceStatus !== 'active' ? 'disabled' : previewState;
  ...
  return (
    <section className={styles.doranPage} aria-labelledby="doran-title">
