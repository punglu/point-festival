...
| Task ID | Wave | Status | Product outcome | Dependencies | Work / DoD |
|---|---|---|---|---|---|
| MONGLE-W2-WAGLE-DURABLE-COMMAND-001 | 2 | `IMPLEMENTED_AWAITING_INDEPENDENT_QA` | idempotent durable send command and history | Wave 1; D4, D6 | Durable send with `client_message_id` idempotency returning the existing result on retry; room and participant rules; message history query. **No user-facing `DELIVERED`** |
| MONGLE-W2-WAGLE-TRANSACTIONAL-OUTBOX-001 | 2 | `IMPLEMENTED_AWAITING_INDEPENDENT_QA` | atomic message + Outbox commit | MONGLE-W2-WAGLE-DURABLE-COMMAND-001; D6 | Message persistence and Outbox enqueue in **one transaction**; sender success reported only after commit; a later realtime or Push failure never loses the message or reverses success. **Start Gate:** verify whether the current implementation already shares one transaction. Tests: forced post-persist failure leaves no divergence |
| MONGLE-W2-WAGLE-ORDERING-CURSOR-001 | 2 | `IMPLEMENTED_AWAITING_INDEPENDENT_QA` | room-scoped ordering, cursor and read state | MONGLE-W2-WAGLE-DURABLE-COMMAND-001; D6 | Monotonic order per `FamilyGroup + Room` via `room_sequence` or equivalent; forward-only read cursor; per-family unread isolation. **Global ordering must not be implemented** |
| MONGLE-W2-WAGLE-ROOM-LIST-READ-MODEL-001 | 2 | `IMPLEMENTED_AWAITING_INDEPENDENT_QA` | batch room list with preview and unread | MONGLE-W2-WAGLE-ORDERING-CURSOR-001 | One call returning rooms plus last-message preview plus unread count; today `list_rooms` returns bare rows. Response shape is this task's design work |

## Wave 3 — Wagle realtime, PWA Push, recovery, PIN

> **Core graduated 2026-08-01** (PM decision on `MONGLE-W3-FRONTEND-RUNTIME-CLOSEOUT-QA-001`'s
> closeout report): the six rows below marked `DONE` are independently
> QA-verified at Backend/API/cross-process layers, plus a live Playwright
> re-run for the frontend surfaces that exist today. `D6-P1`–`D6-P8` rows
> remain `DEFERRED_TO_RELEVANT_TASK_START_GATE`, unchanged and not reopened.
> Five browser-level scenarios (ordinary reconnect, resume, dedup render
> prevention, gap-triggered refetch, per-Family revoke UI isolation) have no
> Playwright coverage yet because `WagleLanding` still renders preview
> fixtures rather than real rooms — carried forward as Wave 6 DoD on
> `MONGLE-W5-TARGET-UI-001` rather than left silently unclaimed.

| Task ID | Wave | Status | Product outcome | Dependencies | Work / DoD |
|---|---|---|---|---|---|
| MONGLE-W2-WAGLE-REALTIME-001 | 3 | `DONE` | realtime dispatcher and WebSocket gateway | Wave 2; D6 | Outbox consumer/dispatcher and WebSocket gateway; one **logical** subscription per `Account + Device + Session` while tolerating multiple concurrent physical sockets, each with its own connection ID and server-side lifecycle and dedup. At-least-once with deduplication. Retry, duplicate and revoke tests. **DONE 2026-08-01** — implemented by `MONGLE-W3-WAGLE-REALTIME-PUSH-RECOVERY-PIN-001`/`MONGLE-W3-WAGLE-MULTIWORKER-FANOUT-001`, independent QA PASS via `MONGLE-W3-WAGLE-INDEPENDENT-QA-001` + `MONGLE-W3-FRONTEND-RUNTIME-CLOSEOUT-QA-001`, PM graduation confirmed |
| MONGLE-W3-WAGLE-PUSH-SUBSCRIPTION-001 | 3 | `DONE` | PWA Web Push subscription and revocation | Wave 2; D6 (payload only: **D6-P1**) | **PM correction 2026-08-01: `D6-P1` gates the payload's user-visible content, not the whole feature.** Subscription lifecycle and delivery infrastructure implemented and independently QA-verified: Account/device subscription lifecycle, register/refresh/unlink, expired-endpoint handling, delivery port and adapter, retry and per-endpoint failure isolation, identifier-only safe envelope, and Push delivery that continues while the device PIN lock is engaged. **DONE for this foundational scope, 2026-08-01.** Still deferred to `D6-P1`/`D6-P2`/`D6-P3`: message preview or body exposure, sender detail, notification grouping, mute defaults, and user-facing notification presentation — those remain their own `DEFERRED_TO_RELEVANT_TASK_START_GATE` rows below, not reopened by this row's graduation |
| MONGLE-W3-WAGLE-RECONNECT-RESUME-001 | 3 | `DONE` | reconnect, resume and recovery | MONGLE-W2-WAGLE-REALTIME-001; D6 | Client-held cursor or resume token; on reconnect the server returns missed events and both sides deduplicate. Survives reconnect, server restart, device sleep, PWA relaunch and Push loss with no loss and no duplicate display. Push is never treated as guaranteed delivery or SSOT. **DONE 2026-08-01** at the Backend/API layer, independently QA-verified (239/239). Browser-level re-proof (real reconnect/resume/dedup/gap-recovery against a real room) is carried forward as Wave 6 DoD on `MONGLE-W5-TARGET-UI-001` once `WagleLanding` renders real rooms instead of preview fixtures — not a reopening of this row |
| MONGLE-W3-WAGLE-MULTIFAMILY-SUBSCRIPTION-001 | 3 | `DONE` | one logical subscription across all authorized families | MONGLE-W2-WAGLE-REALTIME-001; D1, D6 | Multiplex the server-derived `AuthorizedFamilySet`; `ActiveFamilyContext` never suppresses another authorized family; arbitrary client family subscription denied; membership approval and revocation update the subscription incrementally, revoking only that family. **DONE 2026-08-01**, independently QA-verified at the Backend/API layer; browser-level per-Family-revoke-isolation re-proof carried forward as Wave 6 DoD on `MONGLE-W5-TARGET-UI-001` |
| MONGLE-W3-WAGLE-FAILURE-ISOLATION-001 | 3 | `DONE` | per-family and per-device failure isolation | MONGLE-W2-WAGLE-REALTIME-001; D6 | Outbox, cursor, read state, unread count, retry and failure records isolated per FamilyGroup; one family's, device's or event's failure never blocks another's delivery. **DONE 2026-08-01**, independently QA-verified including two HIGH corrections to the shared Outbox (`owner_service` claim isolation, rollback-safe `record_failure`) and a live two-container cross-process fan-out proof |
| MONGLE-W3-WAGLE-PIN-LOCK-001 | 3 | `DONE` | optional Wagle PIN screen lock | Wave 1; D3-PIN-SCOPE | `Account + Device` local lock on the conversation screen; one personal PIN per device across all that account's families; no shared or per-family PIN; not readable by FamilyAdmin; **reset, never recovery**; failure never logs the platform out or blocks other services; while locked, Push and unread counts continue but sensitive bodies stay hidden. **DONE 2026-08-01**, independently QA-verified including a live Playwright re-run (PIN lock does not block `/dashboard`/`/family`). Once a Markpoint frontend route exists (Wave 5 UI), `MONGLE-W5-TARGET-UI-001`'s Wave 6 DoD must also assert Markpoint access is unaffected while PIN-locked — not a reopening of this row |
...
| Task ID | Wave | Status | Product outcome | Dependencies | Work / DoD |
|---|---|---|---|---|---|
| MONGLE-W5-TARGET-UI-001 | 6 | `BLOCKED_BY_DEPENDENCY` | Target product journeys on real APIs | frozen Wave 1–5 contracts per slice | Accessible, responsive FE slices plus Target E2E, including multi-family journeys and Push deep-link context switching. **Start Gate:** the consumed backend contract must be reachable — no slice may claim integration while rendering from static preview fixtures. **Wagle room-realtime browser DoD, merged in by PM decision on 2026-08-01 at `MONGLE-W3-FRONTEND-RUNTIME-CLOSEOUT-QA-001` closeout** (no separate `MONGLE-W6-WAGLE-ROOM-REALTIME-UI-E2E-001` — this is the same-purpose existing row): once real Wagle rooms replace `WagleLanding`'s preview fixtures, Target E2E for this slice must include (1) ordinary network-disconnect reconnect, (2) resume/missed-message recovery, (3) duplicate-event render prevention, (4) sequence-gap-triggered history refetch, (5) per-Family revoke leaving other Families' subscriptions/UI working, and (6) a two-browser send-here/receive-there realtime journey. These five backend-equivalent behaviours are already independently QA-verified at the API/service layer (`MONGLE-W3-WAGLE-INDEPENDENT-QA-001`, 239/239) — this Wave 6 DoD is the browser-level re-proof once a real room exists to test against, not a re-opening of Wave 3. Once a Markpoint frontend route exists (Wave 5 UI), this slice's Wagle PIN-lock journey must also assert Markpoint route/feature access is unaffected while locked. |
