...
실행: python3 tests/api/e2e_scenario_test.py
"""
...
# [제언-B 반영] 플레이어 캐시
PLAYER_MAP: dict[str, dict] = {}    # { "유빈": {"id": 1, "name": "유빈", "role": "child"}, ... }
ADMIN_TOKEN: str = ""
...
def init_cache_and_cleanup():
    ...
    # 관리자 토큰
    admin = next((p for p in players if p["role"] == "admin"), None)
    if not admin:
        print("❌ 관리자 플레이어 없음")
        sys.exit(1)
    resp = requests.post(f"{BASE}/api/auth/login", json={
...
def scenario_a_all(admin_name: str, label: str):
    ...
    cleanup_scenario(label)
    children = [p for p in PLAYER_MAP.values() if p["role"] == "child"]
...
def scenario_d4_batch_copy():
    ...
    resp = requests.post(f"{BASE}/api/missions/batch-copy", json={
...
def main():
    ...
    # 그룹 A: 미션 할당 사이클
    scenario_a_mission_cycle("아빠", "유빈", "A1")
    scenario_a_mission_cycle("아빠", "유현", "A2")
    scenario_a_all("아빠", "A3")
    scenario_a_mission_cycle("엄마", "유빈", "A4")
    scenario_a_mission_cycle("엄마", "유현", "A5")
    scenario_a_all("엄마", "A6")
